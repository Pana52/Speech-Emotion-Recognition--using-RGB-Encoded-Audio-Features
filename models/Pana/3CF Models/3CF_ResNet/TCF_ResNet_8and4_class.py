...
def load_dataset_and_extract_features(data_dir, feature_model, emotions, num_classes):
    features = []
    labels = []
...
def train_and_evaluate(num_classes, hyperparams, features, labels, unfreeze=False, layers_to_unfreeze=3):
    # Apply clustering
...
def train_model(dataset, data_dir, num_classes, emotions, model_mode):
    ...
    data_dir = os.path.join(data_dir, dataset)  # Adjust data directory for specific dataset

    if not os.path.exists(data_dir):
        print(f"Skipping dataset {dataset} as the directory does not exist: {data_dir}")
        return

    features, labels = load_dataset_and_extract_features(data_dir, feature_model, emotions, num_classes)
    ...
    for generation in range(NUM_GENERATIONS):
        ...
        for individual in population:
            accuracy, y_true_labels, y_pred_labels = train_and_evaluate(num_classes, individual, features, labels)
...
def process_dataset(dataset, data_dir):
    models = [
        {'num_classes': 8, 'emotions': ['angry', 'calm', 'disgust', 'fearful', 'happy', 'neutral', 'sad', 'surprised'],
         'model_mode': 'UNFREEZE'},
        {'num_classes': 4, 'emotions': ['angry', 'happy', 'neutral', 'sad'], 'model_mode': 'UNFREEZE_4CLASS'}
    ]
    # Use a pool to run models in parallel
    with multiprocessing.Pool(len(models)) as pool:
        pool.starmap(train_model,
                     [(dataset, data_dir, model['num_classes'], model['emotions'], model['model_mode']) for model in
                      models])
